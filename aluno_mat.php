...
<body class="min-vh-100" style="background-color: #F4F6F8; color: #2D3748;">
    ...
    <nav class="navbar shadow-sm py-3" style="background-color: #FFFFFF;">
        <div class="container d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center gap-3">
                ...
                <h4 class="mb-0 fw-bold" style="color: #DD6B20;">Matemática <span class="badge rounded-pill align-middle ms-2 bg-info text-white" style="font-size: 0.75rem;">Aluno</span></h4>
            </div>
            ...
            <div class="d-flex gap-2">
                ...
                <a href="api/logout.php" class="btn btn-light text-danger fw-bold rounded-pill px-3 shadow-sm border-0">Sair</a>
            </div>
        </div>
    </nav>
    ...
    <div class="container py-5">
        <div class="row mb-5 justify-content-center">
            <div class="col-12 col-md-8 col-lg-6">
                <div class="input-group input-group-lg rounded-pill shadow-sm overflow-hidden" style="background-color: #FFFFFF;">
                    ...
                    <input type="text" id="pesquisaTermo" class="form-control border-0 shadow-none fs-6 py-3" placeholder="Pesquisar no dicionário...">
                </div>
            </div>
        </div>
        ...
        <div class="row g-4" id="listaTermos">
            <div class="col-12 text-center">
                ...
                <p class="mt-2 text-muted">Carregando fórmulas...</p>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg rounded-4">
                <div class="modal-header border-0 pb-0 pt-4 px-4">
                    <h5 class="modal-title fw-bold">Sugerir Nova Fórmula</h5>
                ...
                <form id="formNovaFormulaAluno">
                    ...
                    <div class="modal-body px-4 py-4">
                        <div class="mb-4">
                            <label class="form-label fw-bold small text-uppercase text-muted">Nome da Fórmula</label>
                            <input type="text" class="form-control form-control-lg rounded-3 border-0 bg-light" name="nome_termo" required>
                        </div>
                        <div class="mb-2">
                            <label class="form-label fw-bold small text-uppercase text-muted">Explicação</label>
                    ...
                    <div class="modal-footer border-0 pt-0 pb-4 px-4 d-flex gap-2">
                        ...
                        <button type="submit" id="btnSalvar" class="btn rounded-pill px-4 fw-bold flex-grow-1" style="background-color: #DD6B20; color: #FFFFFF;">Enviar</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="modal fade" id="modalLeituraFormula" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content border-0 shadow-lg rounded-4">
                <div class="modal-header border-0 pb-0 pt-4 px-4">
                    <h4 class="modal-title fw-bold" id="modalTitulo" style="color: #DD6B20;"></h4>
                    <button type="button" class="btn-close shadow-none" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body px-4 py-4">
                    <p id="modalSignificado" class="text-secondary fs-5" style="white-space: pre-wrap;"></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function carregarTermos() {
            ...
            // Busca os termos da API
            fetch('api/aluno.php?acao=GET&disciplina=Matematica')
                .then(res => res.json())
                .then(data => {
                    lista.innerHTML = '';
                    if (data.status === 'sucesso' && data.dados.length > 0) {
                            ...
                            lista.innerHTML += `
                                <div class="col-12 col-md-6 col-lg-4 item-termo">
                                    <div class="card h-100 border-0 shadow-sm rounded-4 card-clicavel" 
                                         style="border-left: 5px solid #DD6B20 !important;"
                                         onclick="abrirFormula('${item.nome_termo.replace(/'/g, "\\'")}', '${item.significado_termo.replace(/'/g, "\\'")}')">
                        ...
                        lista.innerHTML = '<p class="text-center text-muted">Nenhum termo encontrado.</p>';
                    }
                })
                .catch(err => {
                    lista.innerHTML = '<p class="text-center text-danger">Erro ao carregar os dados.</p>';
                    console.error(err);
                });
        }

        // Função para enviar dados
        document.getElementById('formNovaFormulaAluno').addEventListener('submit', function(e) {
            e.preventDefault();
            const btn = document.getElementById('btnSalvar');
            btn.disabled = true;

            const formData = new FormData(this);
            formData.append('acao', 'POST'); // Define a ação para o PHP
            ...
            .then(data => {
                if(data.status === 'sucesso') {
                    bootstrap.Modal.getInstance(document.getElementById('modalNovaFormula')).hide();
                    this.reset();
                    carregarTermos();
                } else {
        ...
        function abrirFormula(titulo, significado) {
            ...
            new bootstrap.Modal(document.getElementById('modalLeituraFormula')).show();
        }
