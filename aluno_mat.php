...
<head>
    ...
    <title>Matemática - Área do Aluno</title>
    ...
    <style>
        ...
        .scroll-alfabeto::-webkit-scrollbar { display: none; }
        .letra-item {
            flex: 0 0 auto;
            width: 48px;
            height: 48px;
...
<body class="min-vh-100" style="background-color: #F4F6F8; color: #2D3748;">
        ...
        <div class="container d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex gap-2 align-items-center">
                <button class="btn fw-bold rounded-pill px-4 shadow-sm" data-bs-toggle="modal" data-bs-target="#modalNovoTermo" style="background-color: #E53E3E; color: #FFFFFF;">
                    <span class="d-none d-sm-inline">+ Contribuir</span>
        ...
        <div class="row mb-5 justify-content-center">
            <div class="col-12 col-md-8 col-lg-6">
                <div class="input-group input-group-lg rounded-pill shadow-sm overflow-hidden" style="background-color: #FFFFFF;">
                    ...
                    <input type="text" id="pesquisaTermo" class="form-control border-0 shadow-none fs-6 py-3" placeholder="Pesquisar equação, conceito..." style="color: #2D3748;">
                </div>
            </div>
        </div>

        <div class="row g-4" id="listaTermos"></div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg rounded-4">
                <div class="modal-header border-0 pb-0 pt-4 px-4">
                    <h5 class="modal-title fw-bold" style="color: #2D3748;">Contribuir com Termo</h5>
                    <button type="button" class="btn-close shadow-none" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form id="formNovoTermo">
                    ...
                    <div class="modal-footer border-0 pt-0 pb-4 px-4 d-flex gap-2">
                        ...
                        <button type="submit" class="btn rounded-pill px-4 fw-bold flex-grow-1" style="background-color: #E53E3E; color: #FFFFFF;">Enviar Sugestão</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        async function carregarTermos() {
            try {
                // Bate na API de Aluno
                const response = await fetch(`api/aluno.php?acao=GET&disciplina=${DISCIPLINA_ATUAL}`);
                    ...
                    <div class="col-12 col-md-6 col-lg-4 mb-4">
                        <div class="card h-100 border-0 shadow-sm rounded-4 position-relative card-clicavel" 
                            ...
                            <div class="card-body p-4 d-flex flex-column">
                                <h5 class="card-title fw-bold mb-0" style="color: #E53E3E; font-size: 1.25rem;">${termo.nome_termo}</h5>
...
document.getElementById('formNovoTermo').onsubmit = async function(e) {
    e.preventDefault();
    console.log("Opa, cliquei em enviar! Pegando os dados..."); 
    ...
    try {
        // Manda os dados para a API
        const res = await fetch('api/aluno.php', { method: 'POST', body: formData });
        
        // Em vez de forçar o JSON direto, pegamos como texto para ver se o PHP cuspiu algum erro!
        const textoResposta = await res.text(); 
        console.log("O servidor respondeu isso aqui:", textoResposta);
        ...
        try {
            ...
            if(data.status === 'sucesso') {
                alert("Boa! Termo enviado com sucesso! 🎉");
                ...
                alert("Aviso do Servidor: " + data.mensagem); 
            }
        } catch (erroJson) {
            alert("Ih, o PHP deu algum erro e quebrou o JSON! Aperte F12 e olhe a aba Console.");
            console.error("O erro no JSON foi:", erroJson);
        }

    } catch (erroFetch) { 
        alert("Erro grave: O JavaScript não conseguiu nem chegar no arquivo api/aluno.php!");
        console.error("Erro:", erroFetch); 
    }
};
    </script>
</body>
